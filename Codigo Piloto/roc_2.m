function [h,AUC,Sspecificity, Ssensitivity] = roc_2(Output, Target, senval, speval, sim_name, silent, h, col)
%function h = roc(Output, Target, senval, speval, sim_name, silent, h, col)
...
%     plot([0 1], [1 0], 'k');
    if ~overlay
        if (~isempty(senval) & ~isempty(speval))
            ...
            title(['AUC=' num2str(round(10000*AUC)/10000)]);
        end
        xlabel('Specificity');
        ylabel('Sensitivity');
        grid on
